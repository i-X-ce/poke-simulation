...
金版 RTA におけるツクシ戦のシミュレーションを行います。
ツクシは虫タイプのジムリーダーで、最初に戦うジムリーダーです。ワニノコのステータス等は一般的な RTA チャートを参考にしています。
...
- **pokemonGold.gb.state**  
  ツクシに話しかけて戦闘が始まり、「ジムリーダーの　ツクシが　しょうぶを　しかけてきた！」が表示されている画面でステートセーブを行ったステートデータです。
...
|No|Type|A|B|HP|Win|Frame|
|-|-|-|-|-|-|-|
|実行番号|0: 通常攻撃, 1: プラスパワー使用|A 個体値|B 個体値|初期 HP|勝利数|フレーム数(勝利時の合計)|
