...
# あばれる
def event_abareru(pyboy: PyBoy, frame=0):
    cnt = 0
    def condition():
        nonlocal cnt
        cnt -= 1
    ...
    frame += wait_until(pyboy, condition, True)
    return frame
...
def event_nirami(pyboy: PyBoy):
    ...
    frame += wait_until(pyboy, condtion, True)
    ...
    frame += wait_until(pyboy, condition, True)
    return event_abareru(pyboy, frame=frame)
...
def trial(A, B, HP, event_type):
    ...
    try:
        ...
        while win < N:
            ...
            if event_type == 0:
                frame = event_abareru(new_pyboy)
            else : # event_type == 1
                frame = event_nirami(new_pyboy)
            # logging.info(f"[PID:{process_id}] Event {event_type} finished for iteration {i+1}")

            if new_pyboy.memory[0xcffd] != 0:
                win += 1
                totalFrame += frame
            if win % 10 == 0 and not multi_thread: tqbar.set_description(f"Type{event_type}, A{A}B{B}HP{HP}, win: {win}/{N}, frame: {totalFrame}f, average: {totalFrame / win: 0.2f}f")
...
if __name__ == "__main__":
    if multi_thread:
        ...
        with open("time_result.csv", "w", newline="") as csvfile:
            print(ABs)
            csvwriter = csv.DictWriter(csvfile, fieldnames=["Type", "Frame"])
            csvwriter.writeheader()
            cnt = 0
            totalStartTime = time.time()
            ...
            A = 15
            B = 15
            HP = nid_states(15, 15, 15, 15)["h"]
            for ET in range(2):
                cnt += 1
                startTime = time.time()
                frame = trial(A, B, HP, ET)
                endTime = time.time()
                print(f"\033[32mET{ET}, frame: {frame}, average: {frame / N: 0.2f}f\033[0m\n", flush=True)
                csvwriter.writerow({"Type": ET, "Frame": frame})
                csvfile.flush()
